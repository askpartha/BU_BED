				...
				<div class="content">	
					...
					?>
						...
						<div class="form-group">
							...
							<?php
									$cors_ctgry = 'id="cors_ctgry" class="col-xs-10 col-sm-10"';				
		
									echo form_dropdown('cors_ctgry', $cors_ctgry_options, null, $cors_ctgry)
								?>
						</div>
...
function printAllCourses(arr) {
	...
	if(arr.length > 0) {
			for(var i=0; i<arr.length; i++) {
				var cors_ctgry= arr[i].cors_ctgry;
				if(cors_ctgry == 'B'){
					cors_ctgry = 'Bachelor';
				}else if(cors_ctgry == 'M'){
					cors_ctgry = 'Masters';
				}
...
$(document).on('click','.edit-record',function() {
	...
	$('#cors_ctgry').val(courses_data[index].cors_ctgry);
...
$(document).on('click','.addedit-btn',function() {
	...
	if(cors_ctgry == ''||  cors_ctgry == 'EMPTY') {
		error = true;
		$('#cors_ctgry').addClass('form-error');
	}
