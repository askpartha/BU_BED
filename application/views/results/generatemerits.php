<div class="page-title">
	...
	<ul class="breadcrumb">
		...
		<li class="">Generate Meritlist</li>
	</ul>
</div>	<!-- /heading -->
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-6">
				<p class="alert alert-success success"  style="display:none;">
					Meritlist of selected category has successfully generated. 
				</p>
				<p class="alert alert-danger failure"  style="display:none;">
					Meritlist of selected category has not successfully generated.
				</p>
		</div>
	</div>
	...
	?>
		...
		<div class="col-sm-3">	
			<div class="form-group">
				<label>&nbsp;</label>
				<button type="button" class="btn btn-success generate-btn">Generate Merit List</button>
			</div>
		</div>
		<?php	
			echo form_close();
...
function loadDropDown(){
	var params = "cand_ctgry="+$("#cand_ctgry").val()+"&type=true";
	$.ajax({
...
function PrintMethodOptiondata(data){
	...
	for(var i=0; i<data.length; i++){
		option_item += '<option value="'+data[i]['method_type']+'">'+data[i]['method_name']+'</option>';
	}
...
$(document).on('click','.generate-btn',function() {
	...
	if(cand_ctgry != "" && cand_ctgry != "EMPTY" ){
		...
		$.ajax({
			url: "<?php echo $this->config->base_url();?>results/generatemeritlist/"+new Date().getTime(),
			type: "post",
			data: params,
			dataType: "html",
			success: function(response){
